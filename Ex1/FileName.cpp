...
void computeArraySumDelay(int array[], int n, int& result) {
	//result = 0;
	for (int i = 0; i < n; i++) {
		int oldValue = result;                 // read
		this_thread::sleep_for(chrono::milliseconds(1)); // delay
		result = oldValue + array[i];          // write
	}
}
...
int main() {
	...
	//we force the race condition appear, delay between read and write
	sum = 0;
	thread tsum11(computeArraySumDelay, values, n, ref(sum));
	thread tsum12(computeArraySumDelay, values2, n, ref(sum));
	tsum11.join();
	tsum12.join();
	cout << endl << "The sum is " << sum;
